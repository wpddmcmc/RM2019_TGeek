...
{ 
  ...
  Image_.create(ACQ_FRAME_HEIGHT, ACQ_FRAME_WIDTH, CV_8UC3);
}
...
GX_STATUS MercureDriver::init_sdk()
{
    ...
    */
    ...
    /* not support
    status_ = GXSetInt(device_, GX_INT_BINNING_HORIZONTAL, 2);
    status_ = GXSetInt(device_, GX_INT_BINNING_VERTICAL, 2);
    status_ = GXSetEnum(device_, GX_ENUM_BINNING_HORIZONTAL_MODE, GX_BINNING_HORIZONTAL_MODE_AVERAGE);
    status_ = GXSetEnum(device_, GX_ENUM_BINNING_VERTICAL_MODE, GX_BINNING_VERTICAL_MODE_AVERAGE);
    status_ = GXSetInt(device_, GX_INT_DECIMATION_HORIZONTAL, 2);
    status_ = GXSetInt(device_, GX_INT_DECIMATION_VERTICAL, 2);
    */
}
...
void MercureDriver::ReadCamera()
{
    ...
    GX_FRAME_DATA *frame_data_ = nullptr;
    while(true)
    {
